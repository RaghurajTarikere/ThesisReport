...
\begin{itemize}
\item Certain number of provided interface ports
\item Certain number of required interface ports
\item Certain number of dataset emitter ports 
\item Certain number of dataset receiver ports
\item Certain number of event emitter ports
\item Certain number of event receiver ports 
\end{itemize}
...
\item [Step 3: Definition of component types] Component types namely \texttt{Component\allowbreak\_Caller} and \texttt{Component\allowbreak\_Callee} which form the basis for a reusable software asset are defined as shown in \cref{fig: Ex. Component types}. 
...
The desired interaction kind for the operations in the required interface ports of \texttt{Component\allowbreak\_Caller} are as shown in \cref{table: NFP RI Ports}
...
For the provided interface slot in the component instance \texttt{Component\allowbreak\_caller\allowbreak\_impl\_inst}, a non-functional property is specified as shown in \cref{table: NFP PI Ports1}
...
For the provided interface slots in the component instance \texttt{Component\allowbreak\_callee\_impl\_inst}, non-functional properties are specified as shown in \cref{table: NFP PI Ports2}
...
For the event receiver slot in the component instance \texttt{Component\allowbreak\_caller\allowbreak\_impl\allowbreak\_inst}, a non-functional property is specified as shown in \cref{table: NFP ER Port}.
...
The following sub-sections also include UML class diagrams wherever appropriate. The standard notations from UML class diagrams are used. The following additional legends are introduced:
\begin{itemize}
...
A data type from the OBSW model is translated into a simple \texttt{typedef} statement from C++ as shown in \cite{typedefRationale}.
...
\begin{itemize}
\item A \texttt{const} non-static protected class variable of name and data type corresponding to the name and data type of interface attribute is added.
